...
/**
 * Rota temporária, super_admin-only: popula conversas demo (kanban de
 * contatos) no tenant informado, para demonstração em produção. Idempotente
 * — não faz nada se o tenant já tiver conversas. Remover após uso.
 */
export async function POST(request: Request) {
  ...
  const existing = await prisma.conversation.count({ where: { tenantId } });
  if (existing > 0) {
    return jsonSuccess({ skipped: true, reason: "Tenant já tem conversas.", conversationCount: existing });
  }
  ...
  return jsonSuccess({ skipped: false, ...result });
}
